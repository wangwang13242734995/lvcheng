...
export default function WeeklyReviewPage() {
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 py-8">
      {/* Header - 深色渐变 */}
      <div className="bg-gradient-to-br from-[#4A3728] to-[#2C1F14] rounded-2xl p-8 mb-8 text-white relative overflow-hidden">
        <div className="absolute top-0 right-0 w-40 h-40 bg-[#5D7A57]/10 rounded-full -translate-y-10 translate-x-10" />
      ...
      <div className="bg-white p-8 rounded-2xl border border-slate-200 mb-8 relative overflow-hidden shadow-sm">
        <div className="absolute left-0 top-0 bottom-0 w-1 bg-gradient-to-b from-[#5D7A57] to-orange-500" />
      ...
      <div className="bg-white p-8 rounded-2xl border border-slate-200 mb-8 shadow-sm">
        ...
        {/* Total Score - 突出展示 */}
        <div className="bg-gradient-to-r from-slate-50 to-[#F7FAF6]/30 p-6 rounded-xl mb-6 border border-slate-100">
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
          {Object.entries(review.abilityChanges.changes).map(([key, change]) => {
            ...
            const currentVal = (review.abilityChanges.current as any)[key] || ABILITY_BASE_SCORE;
            const changeVal = change as number;

            return (
              <div key={key} className="p-4 bg-slate-50 rounded-xl border border-slate-100 hover:border-[#D6E4D2] transition group">
                ...
                <p className="text-2xl font-bold text-slate-800 group-hover:text-[#7A9A75] transition">{currentVal}</p>
              </div>
            );
          })}
        </div>
        ...
        {review.projects.count > 0 ? (
          <div className="space-y-3">
            {review.projects.items.map((p: any) => (
              <Link key={p.id} href={`/projects/${p.id}`} className="block p-5 bg-slate-50 rounded-xl hover:bg-[#F7FAF6]/50 border border-slate-100 hover:border-[#D6E4D2] transition group">
                <div className="flex items-center gap-3">
                  <h3 className="font-semibold text-slate-900 group-hover:text-[#7A9A75] transition">{p.title}</h3>
                  <span className="text-xs bg-slate-200/80 text-slate-600 px-2.5 py-1 rounded-full font-medium">
                    {typeLabels[p.type] || p.type}
                  </span>
                  <span className="ml-auto text-slate-300 group-hover:text-[#8AB382] transition">→</span>
                </div>
              </Link>
            ))}
          </div>
          ...
          <div className="text-center py-8">
            ...
            <Link href="/projects/new" className="inline-flex items-center gap-1 text-[#4A6B43] hover:text-[#7A9A75] font-medium text-sm">
              记录第一个 →
            </Link>
          </div>
        )}
      ...
      {/* CTA - 深色渐变 */}
      <div className="bg-gradient-to-r from-[#4A3728] to-[#2C1F14] rounded-2xl p-8 text-center">
